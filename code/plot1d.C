...
void plot1d(int nsel = 0, int cem = 8){
  ...
  histo->GetXaxis()->SetTitle("H #rightarrow WW #rightarrow 2l, 0 jets, "+  cutLabel[nsel]);
  //labelcms->Draw();
  if (cem == 8)  c1->SaveAs("plots/1D"+cutName[nsel]+ "_8TeV.png");
  else  c1->SaveAs("plots/1D"+ cutName[nsel]+ "_7TeV.png");
  if (cem == 8)  c1->SaveAs("plots/1D"+cutName[nsel]+ "_8TeV.pdf");
  else  c1->SaveAs("plots/1D"+ cutName[nsel]+ "_7TeV.pdf");
